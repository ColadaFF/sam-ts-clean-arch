...
```bash
qonsai-api$ sam local invoke HelloWorldFunction --event events/event.json
```

The SAM CLI can also emulate your application's API. Use the `sam local start-api` to run the API locally on port 3000.

```bash
qonsai-api$ sam local start-api
qonsai-api$ curl http://localhost:3000/
```
...
```bash
qonsai-api$ sam logs -n HelloWorldFunction --stack-name qonsai-api --tail
```
...
```bash
aws cloudformation delete-stack --stack-name qonsai-api
```
